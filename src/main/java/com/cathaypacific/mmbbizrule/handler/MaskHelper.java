...
public class MaskHelper {
	 ...
	 */
	public void mask(FlightBookingDTO flightBookingDTO) {
	 ...
	 * @param flightBookingDTO
	 */
	private void maskAllUserInfo(FlightBookingDTO flightBookingDTO, TokenMaskInfo tokenMaskInfo) {
		List<PassengerDTO> passengers = flightBookingDTO.getPassengers();
		List<PassengerSegmentDTO> passengerSegments = flightBookingDTO.getPassengerSegments();

		String loginMemberPaxId =
				passengers.stream().filter(pax -> BooleanUtils.isTrue(pax.getLoginMember())).map(PassengerDTO::getPassengerId).findFirst().orElse(StringUtils.EMPTY);
	...
	private void maskProductLevel(List<PassengerSegmentDTO> passengerSegments, TokenMaskInfo tokenMaskInfo, String loginMemberPaxId) {

		for (PassengerSegmentDTO passengerSegmentDTO : passengerSegments) {
	 ...
	 * @param passengers
	 */
	private void maskCustomerLevel(List<PassengerDTO> passengers, TokenMaskInfo tokenMaskInfo,String loginMemberPaxId ) {
		...
		for (PassengerDTO passenger : passengers) {
			...
			// contact info
			ContactInfoDTO contactInfo = passenger.getContactInfo();
			maskContactInfoDTO(contactInfo,passenger.getPassengerId(), tokenMaskInfo);
			// emergence contact info
			EmrContactInfoDTO emrContactInfo = passenger.getEmrContactInfo();
	 ...
	 * @param tokenMaskInfo
	 */
	private void maskPriTravelDocDTO(TravelDocDTO travelDocDTO,String passengerId, String segmentId, TokenMaskInfo tokenMaskInfo) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void maskSecTravelDocDTO(TravelDocDTO travelDocDTO,String passengerId, String segmentId, TokenMaskInfo tokenMaskInfo) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void maskRedressDTO(KtnRedressDTO redress, String passengerId, TokenMaskInfo tokenMaskInfo) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void maskKtnDTO(KtnRedressDTO ktn,String passengerId,TokenMaskInfo tokenMaskInfo) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void maskDobDTO(DobDTO dob,String passengerId,TokenMaskInfo tokenMaskInfo) {
	...
	private void maskContactInfoDTO(ContactInfoDTO contactInfoDto, String passengerId, TokenMaskInfo tokenMaskInfo) {
	...
	private void maskEmgerContactInfo(EmrContactInfoDTO emrContactInfo,String passengerId, TokenMaskInfo tokenMaskInfo) {
	...
	private String getAccessableValue(MaskFieldName fieldName, String value,String paxId, String segId, TokenMaskInfo tokenMaskInfo, Consumer<Boolean> maskedSetter){
		...
		if (!value.equals(maskInfo.getOriginalValue())) {
	 ...
	 */
	public TokenMaskInfo unmask(UpdatePassengerDetailsRequestDTO updatePassengerDetailsRequestDTO) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void unmaskPassengerInfo(UpdatePassengerDetailsRequestDTO updatePassengerDetailsRequestDTO,
		...
		if (updatePassengerDetailsRequestDTO != null) {
			...
			UpdateInfantDTO infantDTO = updatePassengerDetailsRequestDTO.getInfant();
			if (infantDTO != null) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void unmaskCustomerLevel(@NotNull UpdatePassengerDetailsRequestDTO passengerDTO,
			@NotNull TokenMaskInfo tokenMaskInfo) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void unmaskProductLevel(@NotNull String passengerId, @NotNull List<UpdateAdultSegmentInfoDTO> segments,
			@NotNull TokenMaskInfo tokenMaskInfo) {

		for (UpdateAdultSegmentInfoDTO segment : segments) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void unmaskInfantCustomerLevel(@NotNull UpdateInfantDTO infantDTO, @NotNull TokenMaskInfo tokenMaskInfo) {
	...
	private void unmaskInfantProductLevel(@NotNull String passengerId,
			@NotNull List<UpdateBasicSegmentInfoDTO> segments, @NotNull TokenMaskInfo tokenMaskInfo) {

		for (UpdateBasicSegmentInfoDTO segment : segments) {
			unmaskPriTravelDocDTO(segment.getPrimaryTravelDoc(), passengerId, segment.getSegmentId(), tokenMaskInfo);
			unmaskSecTravelDocDTO(segment.getSecondaryTravelDoc(), passengerId, segment.getSegmentId(), tokenMaskInfo);
		}
	 ...
	 * @param tokenMaskInfo
	 */
	private void unmaskPriTravelDocDTO(UpdateTravelDocDTO travelDocDTO, String passengerId, String segmentId,
	 ...
	 * @param tokenMaskInfo
	 */
	private void unmaskSecTravelDocDTO(UpdateTravelDocDTO travelDocDTO, String passengerId, String segmentId,
			TokenMaskInfo tokenMaskInfo) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void unmaskRedressDTO(UpdateTsDTO redressDTO, String passengerId, TokenMaskInfo tokenMaskInfo) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void unmaskKtnDTO(UpdateTsDTO ktnDTO, String passengerId, TokenMaskInfo tokenMaskInfo) {
	 ...
	 * @param unlocked
	 */
	private void unmaskPhoneInfoDTO(UpdatePhoneInfoDTO phoneInfoDTO, String passengerId, TokenMaskInfo tokenMaskInfo) {
	 ...
	 * @param tokenMaskInfo
	 */
	private void unmaskEmailDTO(UpdateEmailDTO emailDTO, String passengerId, TokenMaskInfo tokenMaskInfo) {
	...
	private void unmaskEmgerContactInfoDTO(UpdateEmergencyContactDTO emrContactInfoDTO, String passengerId,
			TokenMaskInfo tokenMaskInfo) {
